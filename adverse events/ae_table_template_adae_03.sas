...
*counts;
*by descending frequency of preferred term within alphabetical soc;
*=====================================================================;
...
data final_pre;
	set trans1;
	keep aebodsys aedecod label trt0 trt1 trt2 trt3;
run;

*=======================================================================;
*processing for descending frequency logic;
*=======================================================================;

data final_temp;
	set final_pre;
	tot_count=input(scan(trt3,1,'('),best.);
run;

proc sort data=final_temp;
	by aebodsys descending tot_count aedecod;
run;

data final_temp1;
	set final_temp;
	by aebodsys descending tot_count aedecod;
	if first.aebodsys then soc_ord+1;
	if first.aebodsys then pt_ord=1;
	else pt_ord+1;
run;

proc sort data=final_temp1 out=final;
	by soc_ord aebodsys pt_ord aedecod label;
run;
...
proc report data=final nowd headline headskip missing spacing=0;
	columns soc_ord pt_ord label  trt0 trt1 trt2 trt3;
	define soc_ord/ order noprint;
	define pt_ord/order noprint;
	define label /order "System Organ Class" "  Preferred Term" width=50 flow;
	define trt0/"Placebo" " N=(%cmpres(&n1))" width=21 center ;
	define trt1/"Xanomeline" "50 mg" "N=(%cmpres(&n2))" width=21 center;
	define trt2/"Xanomeline" "80 mg" "N=(%cmpres(&n3))" width=21 center;
	define trt3/"Total" "N=(%cmpres(&n4))" width=20 center;
	break after soc_ord/skip;
