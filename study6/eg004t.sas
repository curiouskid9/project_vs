...
data adeg01;
   set s_adam_i.adeg;
   where saffl="Y" and paramn=5 and avisitn in (6:30);
   if not missing(aval) then
      do;
         if aval le 450 then avalgr2n=1;
         else if 450 lt aval le 480 then avalgr2n=2;
         else if 480 lt aval le 500 then avalgr2n=3;
         else if aval ge 500 then avalgr2n=4;
      end;
run;
...
proc sql noprint;
   ...
   select 0 as avalgr2n,avisitn,avisit,atptn,atpt,trtan,
   count(distinct usubjid) as count 
   from eg 
   where not missing(avalgr2n)
...
proc sql noprint;
   create table abn_count as
      select avisitn,avisit,atptn,atpt,avalgr2n,trtan,
      count(distinct usubjid) as count
      from eg 
      group by avisitn,avisit,atptn,atpt,avalgr2n,trtan;
quit;
...
proc sort data=dummy;
   by avisitn avisit atptn atpt avalgr2n trtan;
run;
...
data counts5;
   set counts4;
   length c2 $30;
   if avalgr2n=0 then c2="n";
   else if avalgr2n=1 then c2="<=450";
   else if avalgr2n=2 then c2=">450";
   else if avalgr2n=3 then c2=">480";
   else if avalgr2n=4 then c2=">500";
run;
...
proc sort data=counts5;
   by avisitn avisit atptn atpt avalgr2n c2 ;
run;
...
data final;
   set trans1;
   keep avisitn avisit atptn atpt avalgr2n c2 trt1 trt2 trt3;
run;
...
proc report data=final nowd headline headskip missing spacing=0;
   columns avisitn atptn c1 avalgr2n c2 c3 c4 c5;
   define avisitn/ order noprint;
   define avalgr2n/order noprint;
