...
data ae_pre01;
   set admh;
   if missing(mhbodsys) then mhbodsys="Not coded";
...
proc sql noprint;
   create table soc_count as
      select mhbodsys, trtan,
      count(distinct usubjid) as count
      from ae 
      group by mhbodsys,trtan;
quit;
...
proc sql noprint;
   create table pt_count as 
      select mhbodsys,mhdecod,trtan,
      count(distinct usubjid) as count
      from ae 
      group by mhbodsys,mhdecod,trtan;
quit;
...
proc sort data=counts out=dummy_pre(keep=mhbodsys mhdecod label) nodupkey;
...
proc sort data=dummy;
   by mhbodsys mhdecod label trtan;
run;
...
data final;
   set trans1;
   keep mhbodsys mhdecod label trt1 trt2 trt3;
run;
...
proc report data=final nowd headline headskip missing spacing=0;
   columns mhbodsys mhdecod c1  c2 c3 c4;
