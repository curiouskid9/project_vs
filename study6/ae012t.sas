...
proc sql noprint;
   create table pt_count as 
      select aebodsys,aedecod,areln,trtan,
      count(distinct usubjid) as count1, count(usubjid) as count2
      from ae 
      where aoccpifl="Y"
      group by aebodsys,aedecod,areln,trtan;
quit;
...
proc sort data=dummy;
   by aebodsys aedecod label areln trtan;
run;
...
proc sql noprint;
   select count(*) into :misscat from counts4_01 where areln=99 and count1 gt 0;
run;

data counts4;
   set counts4_01;
   if (count1=0 and areln=99) and (not &misscat) then delete;
...
data final;
   set trans1;
   length c2 $20;
   if areln=1 then c2="Not Related";
   else if areln=2 then c2="Possible";
   else if areln=3 then c2="Probable";
   else if areln=99 then c2="Missing";
   else if areln=100 then c2="Total";

   keep aebodsys aedecod areln label trt1 trt2 trt3 c2;
run;
...
proc report data=final nowd headline headskip missing spacing=0;
   columns aebodsys aedecod  c1  areln c2 c3 c4 c5;
   ...
   define c2/"Maximum" "Severity" width=15;
