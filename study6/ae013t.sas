...
proc sort data=counts out=dummy_pre(keep=aedecod label) nodupkey;
   by aedecod label;
run;
...
data final;
   set trans1;
   keep aedecod label trt1 trt2 trt3 totfreq ord;
   if label="Overall" then ord=1;
   else ord=2;
run;

proc sort data=final;
   by ord descending totfreq aedecod label;
run;
...
proc report data=final nowd headline headskip missing spacing=0;
   ...
   define c1 /order "System Organ Class" "Preferred Term" width=50 flow;
    define c2/"*redacted;"  "(N=%cmpres(&n1))" width=18  ;
    define c3/ "Placebo" "(N=%cmpres(&n2))" width=18  ;
