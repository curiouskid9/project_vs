...
proc sql noprint;
   select count(*) into :nobs from adsl;
quit;
...
data adsl;
   set adsl;
   keep usubjid trt01an trt01a sexn  aracen age weightbl  heightbl  bmibl saffl;
...
%macro aligdec(indsn=,invar=);
   ...
   %end ;
%mend aligdec;
%aligdec (indsn=final,invar=%str(c3 ));
...
column    c1 group c2 intord c3 ;
    ...
    define c3/"Total" "(N=%cmpres(&n3))" width=18  ;
...
data final;
   length c1-c5 $100;
   call missing(ord, of c1-c3);
   ord=1;
run;

proc report data=final center nowd missing noheader;
   columns ord c1-c5 ;
   define ord/order noprint;
   compute after ord;
      line @35 "No Data Available for This Report";
   endcomp;
run;
